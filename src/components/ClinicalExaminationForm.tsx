    ...
    const renderExtraoral = () => (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-3 md:col-span-2"><span className="text-base font-semibold text-slate-700 block">Jaw Deviation & Opening</span><div className="flex flex-col md:flex-row gap-10"><div className="grid grid-cols-3 gap-3 flex-1">{[{label: 'None', value: 'None'}, {label: 'Left', value: 'To the left'}, {label: 'Right', value: 'To the right'}].map(opt => (<RadioCard key={opt.value} label={opt.label} value={opt.value} currentValue={formData.jawDeviation} onChange={v => updateField('jawDeviation', v)} className="py-2 text-sm text-center whitespace-nowrap" />))}</div><div className="md:w-48"><div className="relative"><Input type="number" value={formData.openingMm} onChange={e => updateField('openingMm', e.target.value)} className="pr-10 bg-slate-50" placeholder="Max Opening" /><span className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-500 text-sm font-medium">mm</span></div></div></div></div>
            </div>
        </div>
    );
