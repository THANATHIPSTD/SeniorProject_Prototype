...
export default function ClinicalExaminationForm() {
    ...
    const { triggerQuickNormal } = useMedicalStore();
    ...
    const MultiSelectCard = ({ label, value, currentValues, onToggle, className }: { label: string, value: string, currentValues: string[], onToggle: (v: string) => void, className?: string }) => {
        ...
        return (
            ...
            >
                <div className={cn("shrink-0 w-5 h-5 rounded flex items-center justify-center border transition-colors", isSelected ? "bg-teal-500 border-teal-500 text-white" : "border-slate-300 bg-white")}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <Input id="allergies" value={formData.allergies} onChange={e => updateField('allergies', e.target.value)} className="bg-slate-50 focus:bg-white" placeholder="e.g., Penicillin, Latex" />
                                    ...
                                    <div className="space-y-2">
                                        <Label htmlFor="dentalHistory" className="text-sm font-semibold text-slate-700">Medical History</Label>
                                        <Input id="dentalHistory" value={formData.dentalHistory} onChange={e => updateField('dentalHistory', e.target.value)} className="bg-slate-50 focus:bg-white" placeholder="e.g., Regular checkup, Relevant past treatments" />
                                    </div>
                </div>
            </div>
    ...
    const renderExtraoral = () => (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-3"><span className="text-base font-semibold text-slate-700 block">Jaw Deviation & Opening</span><div className="flex flex-col gap-3"><div className="grid grid-cols-3 gap-3">{[{label: 'None', value: 'None'}, {label: 'Left', value: 'To the left'}, {label: 'Right', value: 'To the right'}].map(opt => (<RadioCard key={opt.value} label={opt.label} value={opt.value} currentValue={formData.jawDeviation} onChange={v => updateField('jawDeviation', v)} className="py-2 text-sm text-center whitespace-nowrap px-1" />))}</div><div className="w-full"><div className="relative"><Input type="number" value={formData.openingMm} onChange={e => updateField('openingMm', e.target.value)} className="pr-10 bg-slate-50" placeholder="Max Opening" /><span className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-500 text-sm font-medium">mm</span></div></div></div></div>
            </div>
        </div>
    );
